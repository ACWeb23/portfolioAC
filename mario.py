...
def eatFood(foodType):
    ...
    if foodType == POISON:   # Mario eats the Poison Mushroom
        receiveHit()
    if foodType == MAGIC_MUSHROOM:
        mario.super = True
        mario.image = "sumario.png"
    if foodType == ONEUP_MUSHROOM:
        mario.lives += 1
    if foodType == STARMAN:
...
def receiveHit():
    ...
    # if.....   #reacts differently when Mario is invincible, Super, and/or has lives
    if mario.invincible == False:
        if mario.super == True:
            mario.super = False
            mario.image = 'mario.png'
        elif mario.super == False:
           mario.lives -=1
           if mario.lives == 0:
               mario.dead = True
               mario.lives = 0
...
def draw():
    ...
    if mario.dead:
        ...
        screen.draw.text("Invincible timer: "+str(int(mario.invincibleTimer/60)), topleft=(490, 20), fontsize=32)
        # Draw items:
        magicMushroom.draw()
        oneUp.draw()
        starMan.draw()
